...
func TestClock(t *testing.T) {
	...
	if s := c.String(); s != "PWD|Enable|19.2Mhz (54Mhz on RPi4) / 4000.(1509949440/4095)" {
		t.Fatal(s)
	}

	if c.setRaw(clockSrcOscillator, 0) == nil {
		t.Fatal("invalid div")
	}
	if c.setRaw(0, 1) == nil {
		t.Fatal("invalid src")
...
func TestClockCtl_String(t *testing.T) {
	...
	data := []struct {
		...
		{clockMash1 | clockSrcOscillator, "Mash1|19.2Mhz (54Mhz on RPi4)"},
...
func TestCalcSource_exact(t *testing.T) {
	...
	data := []struct {
	...
	}{
		{
			...
			150 * physic.Hertz,
			dmaWaitcyclesMax + 1,
			clockSrcOscillator, 4000, 32,
		},
		{
			200 * physic.Hertz,
			dmaWaitcyclesMax + 1,
			clockSrcOscillator, 4000, 24,
		},
		{
			250 * physic.Hertz,
			dmaWaitcyclesMax + 1,
			clockSrcOscillator, 3840, 20,
		},
		{
			300 * physic.Hertz,
			dmaWaitcyclesMax + 1,
			clockSrcOscillator, 4000, 16,
		},
		{
			1500 * physic.Hertz,
			dmaWaitcyclesMax + 1,
			clockSrcOscillator, 3200, 4,
		},
		{
			1000 * physic.Hertz,
			dmaWaitcyclesMax + 1,
			clockSrcOscillator, 3840, 5,
		},
		{
			2000 * physic.Hertz,
			dmaWaitcyclesMax + 1,
			clockSrcOscillator, 3200, 3,
		},
		{
			2500 * physic.Hertz,
			dmaWaitcyclesMax + 1,
			clockSrcOscillator, 3840, 2,
		},
		{
			3 * physic.KiloHertz,
			dmaWaitcyclesMax + 1,
			clockSrcOscillator, 3200, 2,
		},
		{
			10 * physic.KiloHertz,
			dmaWaitcyclesMax + 1,
			clockSrcOscillator, 1920, 1,
		},
		{
			100 * physic.KiloHertz,
			dmaWaitcyclesMax + 1,
			clockSrcOscillator, 192, 1,
		},
		{
			120 * physic.KiloHertz,
			dmaWaitcyclesMax + 1,
			clockSrcOscillator, 160, 1,
		},
		{
...
func TestCalcSource_inexact(t *testing.T) {
	...
	data := []struct {
	...
	}{
		{
			...
			dmaWaitcyclesMax + 1,
			//clockSrcOscillator, 2121, 31, 292,
		},
		{
		...
		},
		{
			...
			7 * physic.Hertz,
			dmaWaitcyclesMax + 1,
			//clockSrcOscillator, 4095, 32, 146, // 146.52014
		},
	}
	for i, line := range data {
		line := line
...
func BenchmarkCalcSource_Exact(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		if src != clockSrcOscillator || clkDiv != 160 || waitCycles != 1 || actual != 120*physic.KiloHertz || err != nil {
...
func BenchmarkCalcSource_Oversample(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		if src != clockSrcOscillator || clkDiv != 4000 || waitCycles != 32 || actual != 150*physic.Hertz || err != nil {
